...
public class PeliculaService {
	...
	public List<Pelicula> getAllPeliculasByGenero(String genero) throws SQLException,ClassNotFoundException{
		List<Pelicula> peliculas = new ArrayList<Pelicula>();
		
		String genero_lower = genero.toLowerCase();
		
		Connection con = conexion.getConnection();
		String sql = "SELECT * FROM pelicula WHERE genero = ? ";
		
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setString(1, genero_lower);
		ResultSet rs = ps.executeQuery();
	...
	public Pelicula getPelicula(int id) throws SQLException,ClassNotFoundException{
